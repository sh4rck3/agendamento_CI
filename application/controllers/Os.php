...
class Os extends MY_Controller
{
        ...
        if ($this->form_validation->run('os') == false) {
            ...
            if (is_numeric($id = $this->os_model->add('os', $data, true))) {
                ...
                if ($this->data['configuration']['os_notification'] != 'nenhum') {
                    ...
                    $this->enviarOsPorEmail($idOs, $remetentes, 'Agendamento - Criado');
                }

                $this->session->set_flashdata('success', 'Agendamento adicionado com sucesso, você pode adicionar produtos ou serviços a esse Agendamento nas abas de Produtos e Serviços!');
    ...
    public function editar()
    {
        ...
        if (!$this->data['editavel']) {
            $this->session->set_flashdata('error', 'Este Agendamento já e seu status não pode ser alterado e nem suas informações atualizadas. Por favor abrir uma novo Agendamento.');
        ...
        if ($this->form_validation->run('os') == false) {
            ...
            if ($this->os_model->edit('os', $data, 'idOs', $this->input->post('idOs')) == true) {
                ...
                if ($this->data['configuration']['os_notification'] != 'nenhum') {
                    ...
                    $this->enviarOsPorEmail($idOs, $remetentes, 'Agendamento - Editada');
                }


                $this->session->set_flashdata('success', 'AG editada com sucesso!');
    ...
    public function enviar_email()
    {
        ...
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vOs')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para enviar  por e-mail.');
            redirect(base_url());
        }
        ...
        if ($this->data['configuration']['os_notification'] != 'nenhum') {
            ...
            $enviouEmail = $this->enviarOsPorEmail($idOs, $remetentes, 'Agendamento');
    ...
    public function excluir()
    {
        ...
        if ($os->idCobranca != null) {
            if ($os->status == "canceled") {
                ...
                $this->session->set_flashdata('error', 'Existe uma cobrança associada a esta AG, deve cancelar e/ou excluir a cobrança primeiro!');
        ...
        if ((int)$os->faturado === 1) {
            $this->os_model->delete('lancamentos', 'descricao', "Fatura de Agendamento - #${id}");
        }

        log_info('Removeu uma AG. ID: ' . $id);
        $this->session->set_flashdata('success', 'Agendamento excluída com sucesso!');
        redirect(site_url('os/gerenciar/'));
    }
    ...
    public function adicionarProduto()
    {
        ...
        $os = $this->os_model->getById($id);
        if ($os == null) {
            $this->session->set_flashdata('error', 'Erro ao tentar inserir produto na AG.');
            redirect(base_url() . 'index.php/os/gerenciar/');
        }
        ...
        if ($this->os_model->add('produtos_os', $data) == true) {
            ...
            log_info('Adicionou produto a uma AG. ID (AG): ' . $this->input->post('idOsProduto'));
    ...
    public function excluirProduto()
    {
        ...
        $os = $this->os_model->getById($idOs);
        if ($os == null) {
            $this->session->set_flashdata('error', 'Erro ao tentar excluir produto na AG.');
            redirect(base_url() . 'index.php/os/gerenciar/');
        }
        ...
        if ($this->os_model->delete('produtos_os', 'idProdutos_os', $id) == true) {
            ...
            log_info('Removeu produto de uma AG. ID (AG): ' . $idOs);
    ...
    public function adicionarServico()
    {
        ...
        if ($this->os_model->add('servicos_os', $data) == true) {
            log_info('Adicionou serviço a uma AG. ID (AG): ' . $this->input->post('idOsServico'));
    ...
    public function excluirServico()
    {
        ...
        if ($this->os_model->delete('servicos_os', 'idServicos_os', $ID) == true) {
            log_info('Removeu serviço de uma AG. ID (AG): ' . $idOs);
            echo json_encode(['result' => true]);
        } else {
            echo json_encode(['result' => false]);
        }
    ...
    public function anexar()
    {
        ...
        $directory = FCPATH . 'assets' . DIRECTORY_SEPARATOR . 'anexos' . DIRECTORY_SEPARATOR . date('m-Y') . DIRECTORY_SEPARATOR . 'AG-' . $this->input->post('idOsServico');
            ...
            if (!$this->upload->do_upload($field_name)) {
                ...
                if ($upload_data['is_image'] == 1) {
                    ...
                    if (!$this->image_lib->resize()) {
                        ...
                        $success[] = $upload_data;
                        $this->load->model('Os_model');
                        $this->Os_model->anexar($this->input->post('idOsServico'), $upload_data['file_name'], base_url('assets' . DIRECTORY_SEPARATOR . 'anexos' . DIRECTORY_SEPARATOR . date('m-Y') . DIRECTORY_SEPARATOR . 'AG-' . $this->input->post('idOsServico')), 'thumb_' . $upload_data['file_name'], $directory);
                    }
                } else {
                    $success[] = $upload_data;
                    ...
                    $this->Os_model->anexar($this->input->post('idOsServico'), $upload_data['file_name'], base_url('assets' . DIRECTORY_SEPARATOR . 'anexos' . DIRECTORY_SEPARATOR . date('m-Y') . DIRECTORY_SEPARATOR . 'AG-' . $this->input->post('idOsServico')), '', $directory);
                }
            }
        }
        ...
        if (count($error) > 0) {
            ...
            log_info('Adicionou anexo(s) a uma AG. ID (AG): ' . $this->input->post('idOsServico'));
    ...
    public function excluirAnexo($id = null)
    {
        if ($id == null || !is_numeric($id)) {
            ...
            if ($this->os_model->delete('anexos', 'idAnexos', $id) == true) {
                log_info('Removeu anexo de uma AG. ID (AG): ' . $idOs);
    ...
    public function faturar()
    {
        ...
        if ($this->form_validation->run('receita') == false) {
            ...
            $pctm = 50;
            $valor_com_desconto = $this->input->post('valor') - (($this->input->post('valor') * $pctm) / 100);
            $dataComissao = [
                
                'valor' => $valor_com_desconto,
                'idAgendamento' => $this->input->post('os_id'),
                'nomeUsuario' => $this->input->post('funcionario'),
                'dataComissao' => date('Y-m-d'),
                'statusComissao' => 'Pendente',
            ];
            
            $this->os_model->add('usuarios_comissao', $dataComissao);
            ...
            if ($this->os_model->add('lancamentos', $data) == true) {
                ...
                log_info('Faturou uma AG. ID: ' . $os);

                $this->session->set_flashdata('success', 'AG faturada com sucesso!');
                $json = ['result' => true];
                echo json_encode($json);
                die();
            } else {
                $this->session->set_flashdata('error', 'Ocorreu um erro ao tentar faturar AG.');
                $json = ['result' => false];
                echo json_encode($json);
                die();
            }
    ...
    public function adicionarAnotacao()
    {
        ...
        if ($this->form_validation->run('anotacoes_os') == false) {
            ...
            if ($this->os_model->add('anotacoes_os', $data) == true) {
                log_info('Adicionou anotação a uma AG. ID (AG): ' . $this->input->post('os_id'));
                echo json_encode(['result' => true]);
            } else {
                echo json_encode(['result' => false]);
            }
    ...
    public function excluirAnotacao()
    {
        ...
        if ($this->os_model->delete('anotacoes_os', 'idAnotacoes', $id) == true) {
            log_info('Removeu anotação de uma AG. ID (AG): ' . $idOs);
            echo json_encode(['result' => true]);
        } else {
            echo json_encode(['result' => false]);
        }
