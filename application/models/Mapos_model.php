...
class Mapos_model extends CI_Model
{
    ...
    //comissao
    public function getByIdComissao($id)
    {
        $this->db->from('usuarios_comissao');
        $this->db->select('usuarios_comissao.*, usuarios.nome, usuarios.idUsuarios, os.clientes_id, os.horario, os.funcionario, os.usuarios_id, os.dataFinal, clientes.nomeCliente');
        //$this->db->join('permissoes', 'permissoes.idPermissao = usuarios.permissoes_id', 'left');
        $this->db->join('usuarios', 'usuarios.nome = usuarios_comissao.nomeUsuario');
        $this->db->join('os', 'os.idOs = usuarios_comissao.idAgendamento');
        $this->db->join('clientes', 'clientes.idClientes = os.usuarios_id');
    ...
    public function getOsAguardandoMateriais()
    {
        $this->db->select('os.*, clientes.nomeCliente');
        $this->db->from('os');
        $this->db->join('clientes', 'clientes.idClientes = os.clientes_id');
        $this->db->where('os.status', 'Aguardando Materiais');
        $this->db->limit(10);
        return $this->db->get()->result();
    }
